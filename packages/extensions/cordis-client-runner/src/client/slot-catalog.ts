...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'conversation.hero.composer.dock',
    kind: 'list',
    scope: 'session',
    summary: 'Additive entries rendered immediately below the resident Hero InputBar for a blank / zero-turn Session.',
    doc: 'Additive entries rendered immediately below the resident Hero InputBar\nfor a blank / zero-turn Session.\n\nThis seat does not replace the Composer and is not an overlay.\nIt is absent without a real Session.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.hero.composer.dock\', () => ctx.slots.register(\n      { name: \'conversation.hero.composer.dock\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
